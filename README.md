...
The comms module also keeps **transport-agnostic telemetry counters** (`fleece_comms_get_stats`): packets/bytes sent and received plus send/recv failures. It owns no clock, so it reports plain monotonic totals - a host computes rates from its own time source. The received side is credited either by `fleece_comms_receive()` or, for the poll-callback → `fleece_state_manager_import()` pattern, by `fleece_comms_notify_received(size)`.
...
- **World-model hook**: `fleece_goap_brain_set_world_model(brain, cb, ud)` lets the host inject live telemetry into `bb.self` right after each state snapshot (on both the exec and plan/select paths), so pre/goal/eff/exec all see fresh sensor data every tick. This is the platform-agnostic seam - a PX4/MAVLink adapter (or any mock) fills a ring buffer on its own thread and the hook copies the latest sample into the blackboard. Injected fields commit and gossip like any other self field.
- **Bounded search for real-time MCUs**: `fleece_goap_set_max_iters` / `set_max_nodes` / `set_max_plan_depth` cap A* CPU/RAM (defaults 2048 iters, 512 nodes, depth 8). The depth cap also stops the planner from synthesizing silly long detour plans (e.g. reaching `recharge` via a whole forage cycle) when a shorter path exists. `fleece_goap_get_mem_usage()` reports the tables' long-lived footprint for sizing static pools.
- **Eff-vs-exec divergence diagnostics**: `fleece_goap_brain_set_divergence_cb` compares what an action's declared `eff` predicted (the A* model) with what its `exec` body actually produced, per field. Small divergences are normal (eff is a heuristic); this is a debug aid for catching model errors - e.g. an effect claiming a goal becomes reachable while the body never delivers it. Off by default (one extra effect-apply per exec tick while enabled).
...
- **Continuous GPS, no named locations.** `bb.self.x/.y/.home` are real coordinates and the GOAP functions use `Math.hypot` for ranges - the planner's A* simulates geometry. Live telemetry (like a GPS-derived `distHome` so goals such as `charge` are plannable against a moving unit) is injected into `bb.self` each tick via the **world-model hook** (`fleece_goap_brain_set_world_model`) - the same seam a PX4/MAVLink adapter uses: a telemetry thread fills a buffer and the hook copies the latest sample into the brain's blackboard on both the exec and plan/select paths.
